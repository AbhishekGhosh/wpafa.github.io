...
class wpafaeg_PageTemplater {
	...
	public static function get_instance() {
		if (null == self::$instance) {
			self::$instance = new wpafaeg_PageTemplater ();
		}
	...
	private function __construct() {
		...
		$this->templates = array (
				'page-contacts-listing.php' => 'WP-AFA e.g. Contacts',
				'page-donations-listing.php' => 'WP-AFA e.g. Donations' 
		);
	}
	...
	public static function add_plugin_pages() {
		self::add_page ( 'contacts', 'page-contacts-listing.php' );
		self::add_page ( 'donations', 'page-donations-listing.php' );
	}
	/**
	 *
	 * @param
	 ...
	 *        	template_path
	 */
	private static function add_page($page_permalink, $template_path) {
		$the_page = get_page_by_title ( $page_permalink );
		$the_page_id = - 1;
		//
	 ...
	 * Remove API Page
	 */
	public static function remove_plugin_pages() {
		$the_page = get_page_by_title ( 'contacts' );
		if ($the_page) {
			$page_to_delete = $the_page->ID;
			wp_delete_post ( $page_to_delete, true );
		}
		$the_page = get_page_by_title ( 'donations' );
		if ($the_page) {
			$page_to_delete = $the_page->ID;
			wp_delete_post ( $page_to_delete, true );
		}
